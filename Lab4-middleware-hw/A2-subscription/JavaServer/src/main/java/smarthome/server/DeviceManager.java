...
public class DeviceManager implements IDeviceManager {
    ...
    private void initializeDevices() {
        if (serverName.equals("Indoor Server")) {
            ...
            fridge.setMode(RefrigeratorInfo.Types.Mode.NORMAL);
            fridge.setCurrentTemperature(4.0f);
            fridge.setDoorOpen(false);

            List<RefrigeratorCompartment> compartments = new ArrayList<>();

            RefrigeratorCompartment mainCompartment = new RefrigeratorCompartment();
            mainCompartment.setName("Main Compartment");
            mainCompartment.setCurrentTemperature(4.0f);
            mainCompartment.setTargetTemperature(3.0f);
            compartments.add(mainCompartment);

            RefrigeratorCompartment freezerCompartment = new RefrigeratorCompartment();
            freezerCompartment.setName("Freezer Compartment");
            freezerCompartment.setCurrentTemperature(-18.0f);
            freezerCompartment.setTargetTemperature(-20.0f);
            compartments.add(freezerCompartment);
        ...
        if (serverName.equals("Outdoor Server")) {
            ...
            PtzPosition position1 = PtzPosition.newBuilder()
                    .setPan(180)
                    .setTilt(45)
                    .setZoom(5)
                    .build();
            ...
            PtzPosition position2 = PtzPosition.newBuilder()
                    .setPan(90)
                    .setTilt(30)
                    .setZoom(3)
                    .build();
    ...
    @Override
    public boolean tryControlDevice(ControlRequest request, OutParam<String> message) {
        DeviceBase device = getDeviceById(request.getDeviceId());
        if (device == null) {
            message.setValue(String.format("Device with ID %s not found", request.getDeviceId()));
            return false;
        }

        return device.tryHandleControlRequest(request, message);
    }
}